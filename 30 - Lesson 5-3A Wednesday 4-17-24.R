...
#From Friday's class (last set of notes):
#----------------------------------------#
...
#Example Updated to change level of confidence!
CIsim2 <- function(nreps=1000,n=30,loc=0.95){
  #Simulate generating 30 observations from the Normal distribution and using t.test to calculate the lower bound and upper bound of the confidence interval
  mysim <- replicate(nreps,t.test(rnorm(n,100,10),
                                  alternative="two.sided",
                                  mu=100,
                                  conf.level=loc)$conf.int[1:2]) #update 0.95 to new argument, loc
...
#Example Updated to population distribution to Exponential with rate=1/100!
CIsim3 <- function(nreps=1000,n=30,loc=0.95){
  #Simulate generating 30 observations from the Exponential distribution and using t.test to calculate the lower bound and upper bound of the confidence interval
  mysim <- replicate(nreps,t.test(rexp(n,1/100),
                                  alternative="two.sided",
                                  mu=100,
                                  conf.level=loc)$conf.int[1:2]) #update 0.95 to new argument, loc
...
CIsim3(n=50)
CIsim3(n=50,loc=0.90)
CIsim3(n=50,loc=0.99)
